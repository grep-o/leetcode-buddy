...
async function checkExit(ctx: MyContext): Promise<boolean> {
  if (ctx.message && "text" in ctx.message) {
    ...
    if (text === "/exit") {
      await ctx.reply(
        "🚪 Exiting registration. Come back anytime for more coding fun!"
      );
...
export const registrationWizard = new Scenes.WizardScene<MyContext>(
  ...
  async (ctx) => {
    ...
    if (!telegramId)
      return ctx.reply("🤔 How did you hide your id? I can't see you.");

    const user = await userRepo.findByTelegramId(telegramId);
    if (user) {
      await ctx.scene.leave();
      return ctx.reply(
        `Hey ${user.telegramUsername}, you're already registered!`
      );
    }
  ...
  async (ctx) => {
    ...
    if (!ctx.message || !("text" in ctx.message)) {
      await ctx.reply(
        "😕 Oops! I didn't catch that. Please type your LeetCode username."
      );
      return;
    }
    ...
    if (!leetcodeUsername) {
      await ctx.reply(
        "😅 Looks like that was empty. Please send your LeetCode username."
      );
      return;
    }

    await ctx.reply("⏳ Hold on, fetching your LeetCode stats...");
    const info = await getUserLeetcodeInfo(leetcodeUsername).catch(async () => {
      await ctx.reply(
        "😔 Hmm... I couldn't find any LeetCode profile matching that username. Please try again or type /exit to cancel."
      );
    });
    if (!info) return;

    // Save the username in our custom state.
    (ctx.wizard.state as RegistrationWizardState).leetcodeUsername =
      leetcodeUsername;
    ...
    await ctx.reply(
      `🎉 Awesome! I found your profile with a ranking of #${info.profile.ranking}.\n\n` +
        `Now, can you tell me your experience level? (Options: ${Object.values(
          ExperienceLevel
        ).join(", ")})`
    );
    return ctx.wizard.next();
  ...
  async (ctx) => {
    ...
    if (!ctx.message || !("text" in ctx.message)) {
      await ctx.reply(
        "😕 Please type your experience level so we can tailor your journey."
      );
      return;
    }
    const expInput = (ctx.message as { text: string }).text
      .trim()
      .toLowerCase();
    if (!Object.values(ExperienceLevel).includes(expInput as ExperienceLevel)) {
      await ctx.reply(
        "🤔 That doesn't match any valid experience level. Please choose: beginner, intermediate, or advanced."
      );
      return;
    }

    // Save the experience level in our custom state.
    (ctx.wizard.state as RegistrationWizardState).experienceLevel =
      expInput as ExperienceLevel;
    await ctx.reply(
  ...
  async (ctx) => {
    ...
    if (!ctx.message || !("text" in ctx.message)) {
      await ctx.reply(
        "😕 Please type 'daily' or 'weekly' to set your problem-solving interval."
      );
      return;
    }
    const intervalInput = (ctx.message as { text: string }).text
      .trim()
      .toLowerCase();
    if (intervalInput !== "daily" && intervalInput !== "weekly") {
      await ctx.reply(
        "🤔 That doesn't seem right. Please type exactly 'daily' or 'weekly'."
      );
      return;
    }

    // Save the schedule interval in our custom state.
    (ctx.wizard.state as RegistrationWizardState).scheduleInterval =
      intervalInput as "daily" | "weekly";
    await ctx.reply(
  ...
  async (ctx) => {
    ...
    if (!ctx.message || !("text" in ctx.message)) {
      await ctx.reply(
        "😕 Please type a number for how many tasks you want to solve each day."
      );
      return;
    }
    ...
    if (!Number.isFinite(tasksCount) || tasksCount <= 0) {
      await ctx.reply(
        "🤔 That doesn't seem like a valid number. Please enter a positive number (>0)."
      );
      return;
    }
    ...
    const {
      ...
      scheduleInterval,
      leetcodeInfo,
    } = ctx.wizard.state as RegistrationWizardState;
    if (
      !leetcodeUsername ||
      !experienceLevel ||
      !scheduleInterval ||
      ...
      await ctx.reply(
        "😢 Oops! Something went wrong – some of your information is missing. Let's start over."
      );
    ...
    const allSubmissions = leetcodeInfo?.submitStats.acSubmissionNum.find(
